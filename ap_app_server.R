server <- function(input, output, session) {
  ...
  stacks_total <- reactive({
    req(stacks_data())
    sum(stacks_data()[,1][stacks_data()[,1]>0])
  })
  ...
  #source / area detials below chart
  table_1 <- eventReactive(input$btn_table, {
    setNames(data.frame(var_info()$dispname, abs(stacks_data()), pct_col()),
             c(table_title_1(), area_column(), "% of total"))
  })
  ...
  output$whatgroup <- renderUI({
    ...
    #field_names()
    #stacks_data()[1,1]
    paste0(input$group, " ", stacks_name())
    #datfileid()
  })
  ###action button
  ...
  output$table_1 <- DT::renderDataTable({
    DT::datatable(data = table_1(), 
      ...
      DT::formatStyle('% of total',
                      background = DT::styleColorBar(range(table_1()[3], na.rm = TRUE),
                                                     color ='#EFDA5B',
                                                     angle = -90),
